...
public class Grid<GridObject>
{
    ...
    public Grid(int width, int height, float cellSize, Vector3 originPosition, Func<Grid<GridObject>, int, int, GridObject> createGridObject, bool debug)
    {
        this.width = width;
        this.height = height;
        ...
        if (showDebug)
        {
            ...
            for (int x = 0; x < gridArray.GetLength(0); x++)
            {
                for (int y = 0; y < gridArray.GetLength(1); y++)
                {
                    debugTextArray[x, y] = CreateWorldText(gridArray[x, y].ToString(), null, GetWorldPosition(x, y) + new Vector3(cellSize, cellSize) * .5f, 12, Color.white, TextAnchor.MiddleCenter);
